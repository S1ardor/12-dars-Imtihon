const films = [
  ...
  },
  {
    ...
    poster: "https://image.tmdb.org/t/p/w500/gtGCDLhfjW96qconstwctnuTpGOtD.jpg",
    overview:
...
const elResult = document.querySelector(".js-text");
for (let film of films) {
  const newList = document.createElement("li");
  const newFilmImg = document.createElement("img");
  const newTitle = document.createElement("span");
  const newListType = document.createElement("h6");
  const newListCandy = document.createElement("p");

  newFilmImg.src = film.poster;
  newTitle.textContent = film.title;
  newListType.textContent = film.overview;
  newListCandy.textContent = film.genres;

  newList.append(newFilmImg, newTitle, newListType, newListCandy);

  elResult.appendChild(newList);
}
// console.log(films);
